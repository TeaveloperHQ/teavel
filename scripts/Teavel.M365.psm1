...
function Get-TeavelTeamMember {
    ...
    <#
        ...
        <b>여기서 하나하나 세는 것이 지금의 사실이다.</b> Get-UnifiedGroup 의
        GroupMemberCount 는 Exchange 가 세어 둔 값이라 한참 뒤처진다 — 예순 명을 넣고
        나서도 한동안 옛 수를 말한다. 실기에서 우리가 150명을 읽었는데 그쪽은 80명이라고
        했다(2026-08-28).

        그래도 대 보기는 한다. 두 수가 다르면 <b>어느 쪽이 왜 다른지</b> 말해야,
        관리자가 '안 들어갔구나' 하고 또 넣지 않으신다.

        아이디를 못 꺼낸 사람이 있으면 그건 우리 쪽이 덜 읽은 것이므로 따로 말한다.
    #>
    $told = -1
    try {
    ...
    if ($told -ge 0 -and $told -ne $seen.Count) {
        $msg = "$($seen.Count)명이 들어 있습니다."
        $d.Add("NOTE`t지금 센 것은 $($seen.Count)명입니다. 학교 명부에 적힌 수($told 명)는 아직 따라오지 못했습니다 — 그쪽은 시간이 지나면 맞춰집니다.")
    }

    if ($lost -gt 0) {
        $d.Add("NOTE`t$lost 명은 누구인지 알아내지 못해 이 수에서 빠졌습니다 — 사서함이 없는 계정일 수 있습니다.")
    }
