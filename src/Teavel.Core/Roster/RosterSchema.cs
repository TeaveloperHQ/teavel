...
public enum RosterField
{
    ...
    /// <summary>
    /// 표시 이름 — Teams 에서 보이는 이름. <b>학번+이름</b> 꼴이다(예: 10101홍길동).
    /// 열이 없으면 학번과 이름으로 만들어 채운다.
    /// </summary>
    /// <remarks>
    /// 이 규칙은 보기 좋으라고 있는 것이 아니다. <b>학생과 선생님을 가르는 근거</b>다.
    /// 선생님 계정은 이름만 있고 학생은 앞에 학번이 붙으므로, 이름으로 사람을 찾을 때
    /// 둘을 갈라낼 수 있다. 학생을 팀 소유자로 넣으면 그 아이가 반 전체를 지울 수 있다.
    /// (<c>Teavel.M365.TeacherFinder</c> 를 보라)
    /// </remarks>
    DisplayName,
